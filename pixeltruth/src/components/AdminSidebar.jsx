...
import { LayoutDashboard, Users, FileVideo, AlertTriangle, ScrollText, ChevronLeft, ChevronRight, Shield, LogOut } from 'lucide-react';
...
const adminNav = [
  ...
  { to: '/admin/uploads', icon: FileVideo, label: 'All Uploads' },
  { to: '/admin/violations', icon: AlertTriangle, label: 'Violations' },
  { to: '/admin/logs', icon: ScrollText, label: 'Logs & Reports' },
];
...
export default function AdminSidebar({ collapsed, onToggle }) {
  ...
  return (
    ...
    >
        ...
        {adminNav.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              ${isActive ? 'bg-purple-600/20 text-purple-400 border border-purple-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800'}`
            }
          >
      ...
      <div className="px-2 pb-2 space-y-1">
        {!collapsed && user && (
          <div className="px-3 py-2 rounded-lg bg-slate-800/60 border border-slate-700/40">
            ...
            <span className="text-[10px] text-purple-400">Administrator</span>
          </div>
        )}
        <button
        ...
        >
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-sm whitespace-nowrap">Sign Out</motion.span>
            )}
          </AnimatePresence>
        </button>
      </div>
      ...
      <button
        onClick={onToggle}
        className="flex items-center justify-center h-10 border-t border-slate-700/60 text-slate-500 hover:text-purple-400 hover:bg-slate-800 transition-colors"
      >
